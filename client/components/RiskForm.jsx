...
export default function RiskPredictorForm({ onSubmit }) {
  ...
  const submitHandler = async (data) => {
    ...
    try {
      ...
      // Store risk score from server
      setRiskScore(response.data.tenYearRiskPercent);
    } catch (err) {
  ...
  return (
    <div>
      ...
      {/* Display Risk Score */}
      {riskScore !== null && (
        <div className="max-w-lg mx-auto mt-4 bg-white p-4 rounded-lg shadow-md text-center">
          <h3 className="text-lg font-semibold">Predicted 10-Year Risk</h3>
          <p className="text-xl text-indigo-600">{riskScore}%</p>
        </div>
      )}
    </div>
  );
